...
final class LimitStore: ObservableObject {
    ...
    var shortWindow: LimitWindowDescriptor {
        .init(
            id: "short",
            title: "5-hour",
            icon: "timer",
            purpose: "Short window",
            used: shortUsed,
            remaining: shortRemaining,
            resetLabel: "In \(shortResetText)",
            isLimiting: shortUsed >= weeklyUsed,
            accent: AppTheme.statusColor(forUsed: shortUsed)
        )
    }

    var weeklyWindow: LimitWindowDescriptor {
        .init(
            id: "weekly",
            title: "Weekly",
            icon: "calendar",
            purpose: "Plan allowance",
            used: weeklyUsed,
            remaining: weeklyRemaining,
            resetLabel: weeklyResetText,
            isLimiting: weeklyUsed > shortUsed,
            accent: AppTheme.weekly
        )
    }

    var activeWindow: LimitWindowDescriptor {
        shortUsed >= weeklyUsed ? shortWindow : weeklyWindow
    }

    var inactiveWindow: LimitWindowDescriptor {
        shortUsed >= weeklyUsed ? weeklyWindow : shortWindow
    }

    var activeWindowHeadline: String {
        "\(activeWindow.title) \(Int(highestUsed.rounded()))%"
    }

    var activeWindowDetail: String {
        "The menu bar is showing \(activeWindow.title) because \(Int(activeWindow.used.rounded()))% used is higher than \(Int(inactiveWindow.used.rounded()))%."
    }

    var relationshipRuleText: String {
        "Menu bar number = higher used bucket"
    }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 8) {
                    WindowIconBadge(systemName: store.activeWindow.icon, accent: store.activeWindow.accent, size: 28)

                    VStack(alignment: .leading, spacing: 2) {
                        Text(store.activeWindowHeadline)
                ...
                Text(store.activeWindowDetail)
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)

                HStack(spacing: 6) {
                    WindowSummaryPill(window: store.shortWindow)
                    WindowSummaryPill(window: store.weeklyWindow)
                }
            }
...
struct LiveBucketsPanel: View {
    @ObservedObject var store: LimitStore

    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            SectionLabel("Live buckets")

            Text("Drag the buckets directly here. This is the clearest place to compare short-window pressure against weekly pressure.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)

            BucketControlCard(window: store.shortWindow, used: $store.shortUsed)
            BucketControlCard(window: store.weeklyWindow, used: $store.weeklyUsed)
        }
    }
}
...
struct RelationshipPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(store.relationshipRuleText)
                .font(.caption)
                .fontWeight(.semibold)
                .foregroundStyle(.secondary)
            ...
            VStack(spacing: 8) {
                RelationshipStep(
                    index: 1,
                    icon: store.shortWindow.icon,
                    accent: store.shortWindow.accent,
                    title: "Read both buckets",
                    detail: "5-hour is \(Int(store.shortUsed.rounded()))% used. Weekly is \(Int(store.weeklyUsed.rounded()))% used."
                )

                RelationshipStep(
                    index: 2,
                    icon: "arrow.up.right.circle",
                    accent: store.activeWindow.accent,
                    title: "Pick the higher-used bucket",
                    detail: "\(store.activeWindow.title) is closer to the limit than \(store.inactiveWindow.title), so it wins right now."
                )
                ...
                RelationshipStep(
                    ...
                    accent: store.activeWindow.accent,
                    title: "Show that in the menu bar",
                    detail: "The status bar currently shows \(store.activeWindowHeadline)."
                )
            }

            Text("Codex keeps working while both buckets stay below 100%. The menu bar is just a live pointer to whichever bucket is currently tighter.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
...
struct BucketControlCard: View {
    let window: LimitWindowDescriptor
    @Binding var used: Double

    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 10) {
                WindowIconBadge(systemName: window.icon, accent: window.accent, size: 30)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        Text(window.title)
                            .font(.subheadline)
                            .fontWeight(.semibold)
                        if window.isLimiting {
                            Text("Shown now")
                                .font(.caption2)
                                .fontWeight(.semibold)
                                .padding(.horizontal, 6)
                                .padding(.vertical, 2)
                                .background(window.accent.opacity(0.14))
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Slider(value: $used, in: 0...100, step: 1)
                    .tint(window.accent)
            }

            HStack {
                LabelValue(label: "Remaining", value: "\(Int((100 - used).rounded()))%")
                Spacer()
                LabelValue(label: "Used", value: "\(Int(used.rounded()))%")
                Spacer()
                LabelValue(label: "Reset", value: window.resetLabel)
            }
        }
        .padding(12)
        .background(AppTheme.card)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(window.isLimiting ? window.accent.opacity(0.55) : AppTheme.border, lineWidth: 1)
        )
    }
}
...
struct WindowSummaryPill: View {
    let window: LimitWindowDescriptor

    var body: some View {
        HStack(spacing: 6) {
            WindowIconBadge(systemName: window.icon, accent: window.accent, size: 20)
            VStack(alignment: .leading, spacing: 1) {
                Text(window.title)
                    .font(.caption2)
                    .foregroundStyle(.secondary)
        ...
        .padding(.vertical, 6)
        .background(window.accent.opacity(0.10))
        .clipShape(RoundedRectangle(cornerRadius: 8))
    }
}

struct WindowIconBadge: View {
...
struct LimitWindowDescriptor: Identifiable {
    let id: String
    let title: String
    let icon: String
    let purpose: String
...
struct EditorPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("Reset timing + pasted status")
                    .font(.subheadline)
                    .fontWeight(.semibold)
                Spacer()
