...
- Main install CLI: `iconbar`, installable with `pipx install git+https://github.com/Arnie016/codex-goated-skills.git` or [`scripts/install-cli.sh`](https://github.com/Arnie016/codex-goated-skills/blob/main/scripts/install-cli.sh)

Quick commands:

```bash
iconbar search history
iconbar pack list
iconbar pack show daily-briefs-and-reference
iconbar pack show launch-and-distribution
...
Audit snapshot for April 23, 2026:
...
- `64/64` skills are covered by at least one pack

Run the live audit locally:

```bash
iconbar catalog build
iconbar catalog check
iconbar audit
...
- Regenerate it with `iconbar catalog build`.
- Validate freshness with `iconbar catalog check`.
