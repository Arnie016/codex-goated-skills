...
struct RepoOpsLensMenuBarView: View {
    ...
    private let sections = RepoOpsLensDetailView.previewSections
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            chipRow
            detailStack
            RepoOpsLensDetailView(sections: sections)
            actionRow
        }
        .padding(16)
        .frame(width: 360)
        .background(RepoOpsLensTheme.background)
    }

    private var header: some View {
        HStack(alignment: .top, spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 16, style: .continuous).fill(RepoOpsLensTheme.accent)
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Repo Ops Lens")
                    ...
                    .foregroundStyle(.white)
                Text("A menu-bar repo lens for quick audits and planning.")
                    .font(.subheadline)
